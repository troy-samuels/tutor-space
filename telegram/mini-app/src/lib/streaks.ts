/**
 * Cross-game streak management (localStorage + server sync).
 * Streaks persist without login for offline-first gameplay.
 */
...
/**
 * Sync streak data to server (for cross-device persistence).
 * This is called after recordGamePlay when network is available.
 */
export async function syncStreakToServer(userId: number): Promise<void> {
  const data = getStreakData();
  
  try {
    // TODO: Replace with actual API endpoint
    await fetch('/api/streaks/sync', {
      method: 'POST',
    ...
    // Offline-first: local data is still valid, will sync on next connection
  }
}
