...
import { createReferral, getReferralsByReferrer, getReferralByReferee, markReferralBonusAwarded, } from '../src/utils/user-store.js';
...
describe('Referral System', () => {
    ...
    it('should allow same referee to be referred by only one person', async () => {
        ...
        expect(referral?.referrerId).toBe(123); // First referral wins
    });
...
describe('Referral Edge Cases', () => {
    ...
    it('should handle concurrent referral creation', async () => {
        // Simulate concurrent creation
        const promises = [
            createReferral(123, 456),
            createReferral(123, 456),
        ];
        const results = await Promise.all(promises);
        // Both should succeed, but only one should be stored
        const referrals = await getReferralsByReferrer(123);
        expect(referrals).toHaveLength(1);
    });
