...
import { createUser, getUser, updateUser, getAllUsers, createChallenge, getChallenge, updateChallenge, getUserChallenges, } from '../src/utils/user-store.js';
...
describe('Concurrent Access', () => {
    it('should handle concurrent user creation', async () => {
        const promises = Array.from({ length: 5 }, (_, i) => createUser({
            telegramId: 100 + i,
            firstName: `User${i}`,
            joinedAt: new Date().toISOString(),
        }));
    ...
    it('should handle concurrent updates to same user', async () => {
        ...
        // Concurrent updates
        await Promise.all([
            updateUser(123, { streak: 1 }),
            updateUser(123, { totalGames: 5 }),
            updateUser(123, { preferredLanguage: 'spanish' }),
        ]);
        const user = await getUser(123);
        // Last write wins behavior - values should be updated
        expect(user).toBeDefined();
    });
});
