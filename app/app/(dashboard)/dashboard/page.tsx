...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("username, bio, tagline, full_name, plan, email")
    .eq("id", user?.id ?? "")
    .single();
  ...
  const metrics: MetricCardConfig[] = [
    ...
    },
    {
      ...
      helperText: nextBooking
        ? `${(upcomingBookings ?? []).length} lessons upcoming`
        : "Set availability to start booking",
      icon: CalendarDays,
    },
  ];

  const sessions: UpcomingSession[] = (upcomingBookings ?? []).map((booking) => ({
    id: booking.id,
  ...
  return (
    <div className="space-y-8">
      ...
      <ResponsiveSection title="Checklist & next steps">{emptyStatesContent}</ResponsiveSection>
    </div>
  );
}
