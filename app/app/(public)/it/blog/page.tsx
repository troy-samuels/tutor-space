...
export const metadata: Metadata = {
  title: "Blog | TutorLingua - Risorse per Tutor di Lingue Indipendenti",
  description:
    "Guide esperte e strategie per tutor di lingue indipendenti. Impara a ridurre le commissioni delle piattaforme, acquisire più studenti e costruire un'attività di tutoring sostenibile.",
  keywords: [
    "consigli tutoring",
    "blog per tutor di lingue",
    "ridurre commissione Preply",
    "guida attività tutoring",
    "consigli iTalki per tutor",
    "strategie tutoring online",
  ],
  openGraph: {
    title: "Blog TutorLingua - Risorse per Tutor di Lingue",
    description:
      "Guide esperte per tutor di lingue indipendenti. Riduci le commissioni, acquisisci studenti, fai crescere la tua attività.",
    type: "website",
    url: "https://tutorlingua.com/it/blog",
  },
  alternates: {
    languages: {
      en: "/blog",
...
function BlogCard({ post, featured = false }: { post: BlogPostMeta; featured?: boolean }) {
  return (
    ...
    >
      <Link href={`/it/blog/${post.slug}`} className="block h-full">
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {categories.map((category) => (
        <Link
          key={category}
          href={`/it/blog?category=${encodeURIComponent(category.toLowerCase())}`}
          className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${
...
export default function BlogIndexPageIT() {
  const posts = getAllBlogPosts("it");
  const categories = getAllCategories("it");
  const featuredPosts = posts.filter((post) => post.featured).slice(0, 3);
  const recentPosts = posts.slice(0, 12);

  const baseUrl = "https://tutorlingua.com";

  const blogSchema = generateBlogSchema(baseUrl, "it");
  const itemListSchema = generateBlogItemListSchema(posts, baseUrl, "it");
  const breadcrumbSchema = generateBlogIndexBreadcrumbSchema(baseUrl, "it");
  const organizationSchema = generateOrganizationSchema(baseUrl);
  const webSiteSchema = generateWebSiteSchema(baseUrl, "it");
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 backdrop-blur-sm shadow-sm border border-gray-200/50 text-sm font-medium text-gray-700 mb-6">
              ...
              Aggiornato settimanalmente con nuovi contenuti
            </div>
            ...
            <p className="text-lg sm:text-xl text-gray-600 leading-relaxed max-w-2xl mx-auto">
              Guide esperte, strategie e consigli per tutor di lingue indipendenti.
              Impara a far crescere la tua attività, ridurre le commissioni
              e trattenere più guadagni.
            </p>
            ...
            <div className="mt-10 flex flex-wrap items-center justify-center gap-8 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600">Scritto da esperti di tutoring</span>
              </div>
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="mt-2 text-gray-600">
                  Guide selezionate per aiutarti ad avere successo
                </p>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
              ...
              <p className="mt-2 text-gray-600">
                Esplora la nostra libreria completa di risorse
              </p>
            </div>
          ...
          {posts.length === 0 && (
            <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
              ...
              <p className="text-lg font-medium text-gray-900">Nessun articolo trovato</p>
              <p className="text-gray-500 mt-1">Torna presto per nuovi contenuti!</p>
            </div>
          )}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-600 max-w-2xl mx-auto">
              Esplora i nostri contenuti organizzati per gli argomenti più importanti per tutor indipendenti
            </p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <TopicCard
              title="Ridurre Commissioni"
              description="Impara a trattenere più dei tuoi guadagni comprendendo le commissioni delle piattaforme."
              href="/it/blog?category=business"
              count={4}
              icon={
            ...
            />
            <TopicCard
              title="Strumenti e Tecnologia"
              description="Software e sistemi essenziali per gestire un'attività di tutoring professionale."
              href="/it/blog?category=strumenti"
              count={4}
              icon={
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            />
            <TopicCard
              title="Trovare Studenti"
              description="Strategie di marketing e consigli per attirare più studenti alla tua attività."
              href="/it/blog?category=marketing"
              count={3}
              icon={
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <TopicCard
              ...
              description="Best practice per fidelizzare gli studenti e costruire relazioni a lungo termine."
              href="/it/blog?category=retention"
              count={4}
              icon={
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 rounded-3xl p-8 sm:p-12 lg:p-16 text-center">
            ...
            <div className="relative">
              ...
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-4">
                Ricevi Consigli di Tutoring via Email
              </h2>
              <p className="text-lg text-blue-100 mb-8 max-w-2xl mx-auto">
                Unisciti a migliaia di tutor di lingue che ricevono la nostra newsletter
                settimanale con strategie pratiche per far crescere la propria attività.
              </p>
              ...
              <p className="mt-4 text-sm text-blue-200">
                Niente spam, cancellati quando vuoi. Rispettiamo la tua privacy.
              </p>
            </div>
          </div>
        </div>
