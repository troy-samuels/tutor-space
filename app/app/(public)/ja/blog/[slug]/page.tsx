...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const post = await getBlogPost(slug, "ja");

  if (!post) {
    return {
      title: "記事が見つかりません | TutorLinguaブログ",
    };
  }
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/ja/blog/${post.slug}`,
...
export default async function BlogPostPageJA({ params }: PageProps) {
  const { slug } = await params;
  const post = await getBlogPost(slug, "ja");
      ...
      <header className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 text-white">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
          ...
          <nav className="flex items-center gap-2 text-sm text-blue-200/80 mb-8">
            ...
            <Link href="/ja/blog" className="hover:text-white transition-colors">
              ブログ
            </Link>
            <svg className="w-4 h-4 text-blue-300/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-200/70">TutorLinguaチーム</p>
              </div>
            </div>
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm text-blue-200/80">
              ...
              {post.wordCount.toLocaleString("ja-JP")}文字
            </div>
          </div>
      ...
      <article className="relative">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          ...
          <footer className="mt-16 pt-8 border-t border-gray-200">
            {/* Tags */}
            <div className="mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">トピック</h4>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/ja/blog?tag=${encodeURIComponent(tag.toLowerCase())}`}
            ...
            <div className="bg-gray-50 rounded-2xl p-6 mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">この記事をシェア</h4>
              <div className="flex items-center gap-3">
                <a
                ...
                <a
                  href={`mailto:?subject=${encodeURIComponent(post.title)}&body=${encodeURIComponent(`この記事をご覧ください: ${baseUrl}/ja/blog/${post.slug}`)}`}
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`${baseUrl}/ja/blog/${post.slug}`);
                  }}
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 rounded-2xl p-8 sm:p-10 mb-12">
              ...
              <div className="relative">
                ...
                <p className="text-blue-100 mb-6 leading-relaxed">
                  TutorLinguaは直接予約に必要なすべてを提供します：
                  プロフェッショナルな予約ページ、決済処理、
                  自動リマインダー、生徒管理機能。
                </p>
