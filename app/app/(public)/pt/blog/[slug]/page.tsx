...
// Generate static params for all Portuguese blog posts
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const post = await getBlogPost(slug, "pt");

  if (!post) {
    return {
      title: "Artigo Não Encontrado | Blog TutorLingua",
    };
  }
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/pt/blog/${post.slug}`,
      ...
      <header className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 text-white">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
          ...
          <nav className="flex items-center gap-2 text-sm text-blue-200/80 mb-8">
            ...
            <Link href="/pt/blog" className="hover:text-white transition-colors">
              Blog
            </Link>
            <svg className="w-4 h-4 text-blue-300/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-200/70">Equipe TutorLingua</p>
              </div>
            </div>
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm text-blue-200/80">
              ...
              {post.wordCount.toLocaleString("pt-BR")} palavras
            </div>
          </div>
      ...
      <article className="relative">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          ...
          <footer className="mt-16 pt-8 border-t border-gray-200">
            {/* Tags */}
            <div className="mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Temas</h4>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/pt/blog?tag=${encodeURIComponent(tag.toLowerCase())}`}
            ...
            <div className="bg-gray-50 rounded-2xl p-6 mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Compartilhar este artigo</h4>
              <div className="flex items-center gap-3">
                <a
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`${baseUrl}/pt/blog/${post.slug}`)}`}
                  target="_blank"
                ...
                <a
                  href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${baseUrl}/pt/blog/${post.slug}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <a
                  href={`mailto:?subject=${encodeURIComponent(post.title)}&body=${encodeURIComponent(`Confira este artigo: ${baseUrl}/pt/blog/${post.slug}`)}`}
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 rounded-2xl p-8 sm:p-10 mb-12">
              ...
              <div className="relative">
                ...
                <p className="text-blue-100 mb-6 leading-relaxed">
                  TutorLingua oferece tudo que você precisa para aceitar reservas
                  diretas: página de reservas profissional, processamento de pagamentos,
                  lembretes automáticos e gestão de alunos.
                </p>
