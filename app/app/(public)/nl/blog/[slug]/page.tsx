...
// Generate static params for all Dutch blog posts
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const post = await getBlogPost(slug, "nl");

  if (!post) {
    return {
      title: "Artikel Niet Gevonden | TutorLingua Blog",
    };
  }
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/nl/blog/${post.slug}`,
...
export default async function BlogPostPageNL({ params }: PageProps) {
  const { slug } = await params;
  const post = await getBlogPost(slug, "nl");
      ...
      <header className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 text-white">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
          ...
          <nav className="flex items-center gap-2 text-sm text-blue-200/80 mb-8">
            ...
            <Link href="/nl/blog" className="hover:text-white transition-colors">
              Blog
            </Link>
            <svg className="w-4 h-4 text-blue-300/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm text-blue-200/80">
              ...
              {post.wordCount.toLocaleString("nl-NL")} woorden
            </div>
          </div>
      ...
      <article className="relative">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          ...
          <footer className="mt-16 pt-8 border-t border-gray-200">
            {/* Tags */}
            <div className="mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Onderwerpen</h4>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/nl/blog?tag=${encodeURIComponent(tag.toLowerCase())}`}
            ...
            <div className="bg-gray-50 rounded-2xl p-6 mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Deel dit artikel</h4>
              <div className="flex items-center gap-3">
                <a
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`${baseUrl}/nl/blog/${post.slug}`)}`}
                  target="_blank"
                ...
                <a
                  href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${baseUrl}/nl/blog/${post.slug}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <a
                  href={`mailto:?subject=${encodeURIComponent(post.title)}&body=${encodeURIComponent(`Bekijk dit artikel: ${baseUrl}/nl/blog/${post.slug}`)}`}
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`${baseUrl}/nl/blog/${post.slug}`);
                  }}
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 rounded-2xl p-8 sm:p-10 mb-12">
              ...
              <div className="relative">
                ...
                <p className="text-blue-100 mb-6 leading-relaxed">
                  TutorLingua biedt alles wat je nodig hebt voor directe boekingen:
                  professionele boekingspagina, betalingsverwerking,
                  automatische herinneringen en leerlingenbeheer.
                </p>
                  ...
                  {relatedPosts.map((relatedPost) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-500">
                        ...
                        {relatedPost.readingTime} min leestijd
                      </div>
                    </Link>
                  ))}
                </div>
