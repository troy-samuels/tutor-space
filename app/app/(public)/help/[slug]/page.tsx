...
export async function generateStaticParams() {
  const articles = getAllHelpArticles("en");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const article = await getHelpArticle(slug, "en");

  if (!article) {
    return {
      title: "Article Not Found | TutorLingua Help Center",
    };
  }
  ...
  return {
    ...
    alternates: {
      ...
      languages: article.alternateLocale
        ? {
            [article.alternateLocale.locale]: `${baseUrl}/${article.alternateLocale.locale}/help/${article.alternateLocale.slug}`,
          }
        : undefined,
    },
  };
          ...
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-6">
            ...
            {article.alternateLocale && (
              <div className="mt-6 pt-4 border-t border-gray-100">
                <Link
                  href={`/${article.alternateLocale.locale}/help/${article.alternateLocale.slug}`}
                  className="inline-flex items-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors"
                >
                  {article.alternateLocale.locale === "es"
