...
// Generate static params for all Chinese blog posts
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const post = await getBlogPost(slug, "zh");

  if (!post) {
    return {
      title: "文章未找到 | TutorLingua博客",
    };
  }
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/zh/blog/${post.slug}`,
...
export default async function BlogPostPageZH({ params }: PageProps) {
  const { slug } = await params;
  const post = await getBlogPost(slug, "zh");
      ...
      <header className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 text-white">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
          ...
          <nav className="flex items-center gap-2 text-sm text-blue-200/80 mb-8">
            ...
            <Link href="/zh/blog" className="hover:text-white transition-colors">
              博客
            </Link>
            <svg className="w-4 h-4 text-blue-300/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-200/70">TutorLingua团队</p>
              </div>
            </div>
      ...
      <article className="relative">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          ...
          <footer className="mt-16 pt-8 border-t border-gray-200">
            {/* Tags */}
            <div className="mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">主题</h4>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/zh/blog?tag=${encodeURIComponent(tag.toLowerCase())}`}
            ...
            <div className="bg-gray-50 rounded-2xl p-6 mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">分享这篇文章</h4>
              <div className="flex items-center gap-3">
                <a
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`${baseUrl}/zh/blog/${post.slug}`)}`}
                  target="_blank"
                ...
                <a
                  href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${baseUrl}/zh/blog/${post.slug}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <a
                  href={`mailto:?subject=${encodeURIComponent(post.title)}&body=${encodeURIComponent(`查看这篇文章: ${baseUrl}/zh/blog/${post.slug}`)}`}
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`${baseUrl}/zh/blog/${post.slug}`);
                  }}
