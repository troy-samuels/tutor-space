...
export const metadata: Metadata = {
  title: "博客 | TutorLingua - 独立语言教师资源",
  description:
    "为独立语言教师提供的专业指南和策略。学习如何降低平台佣金、吸引更多学生并建立可持续的辅导业务。",
  keywords: [
    "辅导技巧",
    "语言教师博客",
    "降低Preply佣金",
...
function BlogCard({ post, featured = false }: { post: BlogPostMeta; featured?: boolean }) {
  return (
    ...
    >
      <Link href={`/zh/blog/${post.slug}`} className="block h-full">
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {categories.map((category) => (
        <Link
          key={category}
          href={`/zh/blog?category=${encodeURIComponent(category.toLowerCase())}`}
          className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${
...
export default function BlogIndexPageZH() {
  ...
  const breadcrumbSchema = generateBlogIndexBreadcrumbSchema(baseUrl, "zh");
  const organizationSchema = generateOrganizationSchema(baseUrl);
  const webSiteSchema = generateWebSiteSchema(baseUrl, "zh");
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <p className="text-lg sm:text-xl text-gray-600 leading-relaxed max-w-2xl mx-auto">
              为独立语言教师提供的专业指南、策略和技巧。
              学习如何发展业务、降低佣金并保留更多收入。
            </p>
            ...
            <div className="mt-10 flex flex-wrap items-center justify-center gap-8 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600">由辅导专家撰写</span>
              </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {posts.length === 0 && (
            <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
              ...
              <p className="text-lg font-medium text-gray-900">未找到文章</p>
              <p className="text-gray-500 mt-1">敬请期待新内容！</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <TopicCard
              title="降低佣金"
              description="了解平台佣金，学习如何保留更多的辅导收入。"
              href="/zh/blog?category=business"
              count={4}
              icon={
            ...
            <TopicCard
              title="工具和技术"
              description="运营专业辅导业务所需的软件和系统。"
              href="/zh/blog?category=tools"
              count={4}
              icon={
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <TopicCard
              title="吸引学生"
              description="吸引更多学生到您辅导业务的营销策略和技巧。"
              href="/zh/blog?category=marketing"
              count={3}
              icon={
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <TopicCard
              title="学生管理"
              description="留住学生和建立长期关系的最佳实践。"
              href="/zh/blog?category=retention"
              count={4}
              icon={
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
