...
// Generate static params for all German blog posts
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const post = await getBlogPost(slug, "de");

  if (!post) {
    return {
      title: "Artikel Nicht Gefunden | TutorLingua Blog",
    };
  }
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/de/blog/${post.slug}`,
...
export default async function BlogPostPageDE({ params }: PageProps) {
  const { slug } = await params;
  const post = await getBlogPost(slug, "de");
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm text-blue-200/80">
              ...
              {post.wordCount.toLocaleString("de-DE")} Wörter
            </div>
          </div>
      ...
      <article className="relative">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          ...
          <footer className="mt-16 pt-8 border-t border-gray-200">
            {/* Tags */}
            <div className="mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Themen</h4>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/de/blog?tag=${encodeURIComponent(tag.toLowerCase())}`}
            ...
            <div className="bg-gray-50 rounded-2xl p-6 mb-8">
              <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Artikel teilen</h4>
              <div className="flex items-center gap-3">
                <a
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`${baseUrl}/de/blog/${post.slug}`)}`}
                  target="_blank"
                ...
                <a
                  href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${baseUrl}/de/blog/${post.slug}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <a
                  href={`mailto:?subject=${encodeURIComponent(post.title)}&body=${encodeURIComponent(`Schauen Sie sich diesen Artikel an: ${baseUrl}/de/blog/${post.slug}`)}`}
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`${baseUrl}/de/blog/${post.slug}`);
                  }}
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 rounded-2xl p-8 sm:p-10 mb-12">
              ...
              <div className="relative">
                ...
                <p className="text-blue-100 mb-6 leading-relaxed">
                  ...
                  professionelle Buchungsseite, Zahlungsabwicklung,
                  automatische Erinnerungen und Schülerverwaltung.
                </p>
                  ...
                  {relatedPosts.map((relatedPost) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 text-sm text-gray-500">
                        ...
                        {relatedPost.readingTime} Min. Lesezeit
                      </div>
                    </Link>
                  ))}
                </div>
