      ...
      <div className="mb-2 flex items-center justify-between px-1">
        {isActive ? (
          ...
          <span className="text-xs" style={{ color: "#9C9590" }}>
            ↑ Tap a cipher letter above to start decoding
          </span>
        )}
        {isActive && (
          <button
                ...
                {row.map((letter) => {
                  ...
                  return (
                    <motion.button
                      ...
                      onClick={() => isActive && onLetterPick(letter)}
                      disabled={!isActive}
                      className="flex min-h-[44px] min-w-[2.75rem] items-center justify-center rounded-lg text-sm font-semibold transition-colors touch-manipulation select-none cursor-pointer disabled:cursor-default"
                      style={
                        isCorrect
                          ? {
