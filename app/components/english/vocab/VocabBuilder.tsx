...
import { getDailyWord, type VocabWord } from "./VocabData";
...
function PhoneticBadge({ phonetic }: { phonetic: string }) {
  return (
    <span className="font-mono text-sm text-foreground/50 bg-black/5 px-2 py-0.5 rounded-md">
      {phonetic}
    </span>
  );
}
...
function WordCard({ word }: { word: VocabWord }) {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            ...
            <div className="flex items-center gap-2">
              <PhoneticBadge phonetic={word.phonetic} />
              <span className="text-xs text-foreground/50 italic">{word.partOfSpeech}</span>
            </div>
          </div>
          <span
        ...
        <div className="bg-white rounded-2xl border border-black/8 p-4 shadow-soft">
          ...
          <div className="flex flex-wrap gap-1.5">
            {word.synonyms.map((s) => (
              <span
                key={s}
                className="text-xs bg-emerald-50 text-emerald-700 border border-emerald-200/60 px-2 py-0.5 rounded-full"
              >
                {s}
              </span>
            ))}
        ...
        <div className="bg-white rounded-2xl border border-black/8 p-4 shadow-soft">
          ...
          <div className="flex flex-wrap gap-1.5">
            {word.antonyms.map((a) => (
              <span
                key={a}
                className="text-xs bg-red-50 text-red-700 border border-red-200/60 px-2 py-0.5 rounded-full"
              >
                {a}
              </span>
            ))}
      ...
      {/* Collocations */}
      <div className="bg-white rounded-2xl border border-black/8 p-4 shadow-soft">
        <p className="text-xs font-semibold text-foreground/40 uppercase tracking-wider mb-2.5">
          Common Collocations
        </p>
        <div className="flex flex-wrap gap-2">
          {word.collocations.map((c) => (
            <span
              key={c}
              className="text-xs bg-blue-50 text-blue-700 border border-blue-200/60 px-3 py-1 rounded-full"
            >
              {c}
            </span>
          ))}
    ...
    >
      <div className="bg-white rounded-2xl border border-black/8 p-5 shadow-soft">
        ...
        <p className="text-base font-semibold text-foreground">
          What does &ldquo;{word.word}&rdquo; mean?
        </p>
      </div>
      ...
      {revealed && (
        ...
        >
          {selected === word.correctOption
            ? "🎉 Correct! Great work."
            : `❌ Not quite. The answer is: "${word.quizOptions[word.correctOption]}"`}
        </motion.div>
      )}
    </motion.div>
  );
...
export default function VocabBuilder() {
  const [word] = React.useState<VocabWord>(() => getDailyWord());
  ...
  function handleShare() {
    const text = `📖 Word of the Day: ${word.word}\n"${word.definition}"\n\n${word.example}\n\nLearn a new word every day → tutorlingua.co/english/vocab`;
    if (navigator.share) {
      navigator.share({ title: `Word of the Day: ${word.word}`, text }).catch(() => {});
    } else {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Word of the Day</h1>
      ...
      <AnimatePresence mode="wait">
        {phase === "word" && (
          <motion.div key="word" className="flex flex-col gap-5">
            <WordCard word={word} />
            <Button
              size="lg"
              ...
              previewContent={
                <div className="text-center py-2">
                  ...
                  <p className="text-sm text-foreground/60">
                    {quizResult ? "✅ Correctly defined!" : "You've learned a new word!"}
                  </p>
                </div>
              }
              benefit="Get a new advanced word in your inbox every morning — completely free"
        ...
        {phase === "done" && (
          ...
          >
            ...
            <div className="bg-white rounded-2xl border border-black/8 p-6 text-center shadow-soft">
              ...
              <p className="text-sm text-foreground/60">
                {quizResult
                  ? `You correctly identified the meaning of "${word.word}".`
            ...
            <div className="bg-amber-50 border border-amber-200/60 rounded-2xl p-4">
              <p className="text-sm text-amber-800">
                📬 <strong>Check back tomorrow</strong> for a new word. We&apos;ll email
                it to you too!
              </p>
            </div>

            <div className="flex flex-col gap-2.5">
              <Button onClick={handleShare} size="lg" className="w-full rounded-xl min-h-[48px]">
                📋 Share Today&apos;s Word
              </Button>
              <Button
